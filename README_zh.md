...
该数据集作为以下硕士学位论文相关学术研究的一部分公开发布：

《拧紧过程质量管理数字化方案设计与价值实现研究》
...
为保证数据在工艺条件上的一致性，每一个子数据集均来源于同一生产工位、同一拧紧工具以及同一拧紧程序下的实际拧紧结果，不同子数据集之间对应不同的拧紧点位或工况条件。针对各数据集中具有代表性的异常类型，结合现场工程经验，对其中**少量**样本进行了人工标注，用以支持异常分析方法的验证与对比研究。考虑到实际拧紧过程中可能同时出现多种异常现象，数据集采用多标签标注方式，以更真实地反映工业现场的复杂性。
...
因 GitHub 的文件大小限制，原始数据未存储在本代码仓库中，可从以下地址获取：
